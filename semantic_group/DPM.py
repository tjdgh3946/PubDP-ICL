...
class DPM:
    def __init__(
        ...
        delta,
        *,
        epsilon_scale_exp=0.20,
        epsilon_scale_average=0.80,
        t=0.3,
        q=1 / 12,
        num_splits_levels=7,
        debug=False,
        **unused_kwargs,
        ...
        self.data = data
        self.epsilon_bandswidth = 10000.0
        self.epsilon_count = 10000.0  # epsilon * epsilon_scale_count
        ...
        self.w_c = 1.0
        self.w_e = 5.0
        self.num_split_levels = num_splits_levels
    ...
    def perform_clustering(self):
        ...
        # Heuristic
        min_bound, max_bound = self.bounds[0], self.bounds[1]

        # Ensure at least 3 intervals
        range_size = max_bound - min_bound
        min_intervals = 3

        # Adjust interval size dynamically if necessary
        if self.interval_size > range_size:
            est_interval_size = self.interval_size
            self.interval_size = range_size / (2 / 3 * (min_intervals - 1))
            if self.debug:
                print(
                    f"Interval size is adjusted from {est_interval_size} to {self.interval_size}"
                )
...
def argmin_idx(center, embeddings):
    distances_to_centroid = np.linalg.norm(embeddings - center, axis=1)

    return np.argmin(distances_to_centroid)
...
if __name__ == "__main__":
    ...
    subpath = "semantic_group/Summarization_davinci-002_100way-4shot_aggregate/"
    # subpath = (
    #    "semantic_group/medical-question-answering-datasets_chatdoctor_icliniq_gpt-3"
    # )
    eps = 1.0

    for i in tqdm(range(100)):
        path = os.path.join(subpath, f"{i}.npy")
        ...
        # embedding.shape = (num_ensemble, embedding_dim)
        num_points, embedding_dim = embeddings.shape[0], embeddings.shape[1]
        lb, ub = np.min(embeddings), np.max(embeddings)
        # lb, ub = -1.0, 1.0
        # eps_GM = [0.5, 2.49, 7.51]  eps_EM = [0.12, 0.12, 0.12]
        eps_EM, eps_GM = 0.12, 7.51
        # eps_inf_EM, eps_inf_GM = 10.0, 10.0
        ...
        dpm = DPM(
            ...
            bounds=(
                lb,
                ub,
            ),  # Due to the noisy percentile, this is more stable.
            epsilon=eps,
            epsilon_scale_exp=eps_EM / eps,
            epsilon_scale_average=eps_GM / eps,
            delta=1 / (np.sqrt(num_points) * num_points),
            num_splits_levels=7,
        )

        centers, clusters = dpm.perform_clustering()
        public_embeddings = embeddings[100:]
        ...
        #    closest_idx = argmin_idx(center, public_embeddings)

        """
        np.save(os.path.join(subpath, f"centers_{i}.npy"), centers)
        for j in range(len(clusters)):
            cluster_subpath = os.path.join(subpath, "clusters")
            np.save(os.path.join(cluster_subpath, f"{i}_{j}.npy"), clusters[j])
        """
